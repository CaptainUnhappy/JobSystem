...
def processing_data(data, category):
    ...
    for each in jobs:
        ...
        if isLog:
            print(job_id, job_name, salary, degree, categories, major, welfare, head_count, publish_date, update_date,source,company_name, area,company_scale, company_property)
...
def saveData(job_id, job_name, salary, degree, categories, major, welfare, head_count, publish_date, update_date,
    ...
    # 定义最大长度
    max_major_length = 500  # 数据库中 major 字段的最大长度
    # 检查并截断 major 字段
    if len(major) > max_major_length:
            print(f"{job_id} major 字段过长，截断到 {max_major_length} 个字符")
            major = major[:max_major_length]  # 截断到最大长度
...
if __name__ == '__main__':
    ...
    # 处理数据，并存入数据库
    # https://www.ncss.cn/student/jobs/{job_id}/detail.html

    isLog = False
    for index in range(1, 30):
        for i in range(1, 10):
            ind = f"{index:02d}"
            print(f"获取到 {ind} 第{i}页")
            url = f"https://www.ncss.cn/student/jobs/jobslist/ajax/?&limit=20&offset={i}&categoryCode={ind}"
