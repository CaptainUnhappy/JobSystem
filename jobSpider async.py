...
async def get_page_data(
    url: str, session: aiohttp.ClientSession, semaphore: Semaphore
) -> Optional[str]:
        ...
        async with conn.cursor() as cursor:
            try:
                ...
                await cursor.execute(
                    check_sql,
                    (job_data["job_name"], job_data["area"], job_data["company_name"]),
                )
        ...
        for job in jobs:
            ...
            job_data = {
                ...
                "publish_date": time.strftime(
                    "%Y-%m-%d", time.localtime(int(job["publishDate"] / 1000))
                ),
                "update_date": time.strftime(
                    "%Y-%m-%d", time.localtime(int(job["updateDate"] / 1000))
                ),
            }
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=conn) as session:
        ...
        if timeout_urls:
            ...
            retry_tasks = [
                asyncio.create_task(get_page_data(url, session, semaphore))
