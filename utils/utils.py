...
def get_industry_data():
    ...
    for i in range(0, 8):
        ...
        # 获取上个月的数据个数
        if index in last.groups:  # 检查 index 是否在上个月的分组中
            last_group_len = len(last.get_group(index))
        else:
            last_group_len = 0  # 如果不在，则假设上个月数据量为 0

        if last_group_len == 0:
            if now_group_len > 0:  # 上月为0，本月大于0，大幅增长
                present = (
                    f'<th scope="row">{i + 1}</th><td>{index}</td><td><span class="label label-success">{now_group_len}'
                    f'</span></td><td><h5>新增<i class="fa fa-level-up"></i></h5></td>'
                )  # 或者显示 100%+ ，但 "新增" 更清晰
            else:  # 上月为0，本月也为0，无变化
                present = (
                    f'<th scope="row">{i + 1}</th><td>{index}</td><td><span class="label label-info">{now_group_len}'
                    f"</span></td><td><h5>---<i></i></h5></td>"
                )
        elif now_group_len > last_group_len:  # 增加了
            present = (
                f'<th scope="row">{i + 1}</th><td>{index}</td><td><span class="label label-success">{now_group_len}'
            ...
            present = (
                f'<th scope="row">{i + 1}</th><td>{index}</td><td><span class="label label-danger">{now_group_len}'
...
def get_degree_data():
    ...
    degree_num = []
    degree_salary = []
    degree_count = ag.size()  # 获取分组大小
    for i in range(min(5, len(degree_count))):  # 确保不会超出索引范围
        index = degree_count.index[i]
        index_group = ag.get_group(index)
        num = len(index_group)
        dic = {"value": num, "name": index}
        degree_num.append(dic)
        salary = index_group["salary"].astype(float).mean()
        dic2 = {"value": round(numpy.nan_to_num(salary), 2), "name": index}
...
def get_cloud_data():
    ...
    for key in count.keys():
        cloud_data.append({"name": key, "value": count[key]})
    return cloud_data
...
def get_realtime_data():
    ...
    condition = df["publish_date"].str.contains(today)
    realtime_list = []
    filtered_df = df[condition]  # 过滤数据
    if filtered_df.empty:  # 检查过滤后的 DataFrame 是否为空
        info = "未找到今日数据"
        print(info)
        item = {
            "img": "../static/images/"
        ...
        result = filtered_df.sample(n=10).values
        for job in result:
            if len(job[1]) > 10:
                continue
            item = {
...
def get_inclination():
    ...
    for time in time_list:
        ...
        # 跳过缺失值，只对纯数值行列计算
        pd.set_option("display.precision", 2)
        salary = d["salary"].astype(float).mean(skipna=True, numeric_only=True)
        salary = numpy.nan_to_num(salary)
